...
const OrganisationAddImg: React.FC = () => {
const [images, setImages] = useState<ImageType[]>([]);


const fetchImages = async () => {
const res = await api.organization_api.get<ImageType[]>(`/images?organisation_id=${organisationId}`);
setImages(res.data);
};
  ...
  const handleImageUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files) {
      ...
      const res = await api.organization_api.post("/upload", formData, {
  ...
  const deleteImage = async (id: string) => {
    await api.organization_api.delete(`/image/${id}`);
  ...
  return (
    <div className="center-page">
      <div className="dashboard-container">
        ...
        <h2>Add Organisation Images Here</h2>
...
export default OrganisationAddImg;
