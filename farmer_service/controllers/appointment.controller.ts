...
import { DoctorModel } from "../models/availability.model";
import axios from 'axios';


export const bookAppointment = async (req: BookAppointment, res: Response): Promise<void> => {
  const { doctorId, start_date, start_time, end_date, end_time, farmerName, farmerContact, type,species, praniAadharNumber, razorpay_payment_id, healthRecord,} = req.body;
...
export const cancelAppointment = async (req: BookAppointment, res: Response): Promise<void> => {
  try {
    ...
    try {
      await axios.post('https://server.pranimithra.in/pay/api/payment/refund', {
        farmer_id: updatedAppointment.farmer_id,
        appointment_id: updatedAppointment._id,
        // amount: updatedAppointment.amountPaid, // optional
      });
    } catch (refundErr: any) {
      console.error('Failed to trigger refund:', refundErr.response?.data || refundErr.message);
      // You can still proceed, or flag refund as pending
    }
